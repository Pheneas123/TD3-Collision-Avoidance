...
def policy_evaluation(
    ...
    *,
    ...
    for ep in tqdm(
        range(n_eval_episodes), desc=f"Evaluating {name}".strip() or "Evaluating"
    ):
        ...
        if save_gif and ep == 0:
            gif_name = (
                f"{name.lower().replace(' ', '_')}_eval_episode_1.gif"
                if name
                else "eval_episode_1.gif"
            )
    ...
    print(f"[{name}] Success Rate: {success_count / n_eval_episodes * 100:.2f}%")
    ...
    plot_filename = (
        f"{name.lower().replace(' ', '_')}_eval_rewards.png"
        if name
        else "eval_rewards.png"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--actor", type=str, default=None, help="Path to td3_actor.pth (optional)"
    )
    ...
    parser.add_argument(
        "--gif", action="store_true", help="Save a GIF for the first eval episode"
    )
    args = parser.parse_args()

    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    save_dir = (
        Path(args.save_dir) / f"{args.name.lower().replace(' ', '_')}_{timestamp}"
    )
